...
class Restaurant:
    ...
    def fromJSON(cls, data):
        ...
        r.preferences.append(DietaryPreference("vegan", data["vegan"] == "yes"))
        r.preferences.append(DietaryPreference("gluten", data["glutenfree"] == "yes", inverse=True))
        r.preferences.append(DietaryPreference("meat", data["meat"] == "yes"))
        return r
...
class DietaryPreference:
    ...
    inverse:   See examples above, but it inverts the choice, so:
               ...
               DietaryPreference("thai", False, True) -> Prefer not thai DietaryPreference("thai", True True) -> Cannot have thai(loser)
    """
...
class GetALunchSpotIntent:
    def __init__(self, alexa_intent):
        slotmap = alexa_intent.get_slot_map()
        self.people = ""
        if slotmap.has_key("Person"):
            self.people = slotmap["Person"].split()
...
with open('restaurants.json') as data_file:
    ...
    restaurants = [Restaurant.fromJSON(r) for r in data]
